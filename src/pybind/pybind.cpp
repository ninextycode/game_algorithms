...
//              py::arg("wrapped_node"), py::arg("p_random_choice"), py::arg("max_random_actions") = -1);
    ...
    // InfoSet
    py::class_<RegretData>(m, "InfoSet")
        .def(py::init<int>())
        .def(py::init<const RegretData&>())
        .def("getInstantRegret", &RegretData::getInstantRegret,
             py::return_value_policy::reference_internal)
        .def("getRegretSum", &RegretData::getRegretSum,
             py::return_value_policy::reference_internal)
        .def("getRegretSumStrategy",
             py::overload_cast<>(&RegretData::getRegretSumStrategy, py::const_))
        .def("getCumulativeStrategy",
             py::overload_cast<>(&RegretData::getCumulativeStrategy, py::const_))
        .def("setInstantRegret", &RegretData::setInstantRegret)
        .def("accumulateRegret", &RegretData::accumulateRegret)
        .def("accumulateStrategy", &RegretData::accumulateStrategy);

    // RegretDataMap types - factory functions that return new dict instances
    ...
    // InfoSet utilities
    py::class_<regret_data_utils::RegretMetric>(m, "RegretMetric")
        .def_readwrite("n_regrets", &regret_data_utils::RegretMetric::n_regrets)
        .def_readwrite("max_instant_regret", &regret_data_utils::RegretMetric::max_instant_regret)
        .def_readwrite("sum_positive_instant_regrets", &regret_data_utils::RegretMetric::sum_positive_instant_regrets);

    // InfoSet SaveLoader utilities
    py::class_<regret_data_utils::SaveLoader>(m, "InfoSetSaveLoader")
        .def_static("saveInfoSetMapStrategy", 
                    py::overload_cast<const RegretDataMap<string>&, const string&>(
                        &regret_data_utils::SaveLoader::saveInfoSetMapStrategy<string>
                    ))
        .def_static("saveInfoSetMapStrategy", 
                    py::overload_cast<const RegretDataMap<size_t>&, const string&>(
                        &regret_data_utils::SaveLoader::saveInfoSetMapStrategy<size_t>
                    ))
        .def_static("saveInfoSetMapRegretSum", 
                    py::overload_cast<const RegretDataMap<string>&, const string&>(
                        &regret_data_utils::SaveLoader::saveInfoSetMapRegretSum<string>
                    ))
        .def_static("saveInfoSetMapRegretSum", 
                    py::overload_cast<const RegretDataMap<size_t>&, const string&>(
                        &regret_data_utils::SaveLoader::saveInfoSetMapRegretSum<size_t>
                    ))
        .def_static("saveInfoSetMapInstantRegret", 
                    py::overload_cast<const RegretDataMap<string>&, const string&>(
                        &regret_data_utils::SaveLoader::saveInfoSetMapInstantRegret<string>
                    ))
        .def_static("saveInfoSetMapInstantRegret", 
                    py::overload_cast<const RegretDataMap<size_t>&, const string&>(
                        &regret_data_utils::SaveLoader::saveInfoSetMapInstantRegret<size_t>
                    ))
        .def_static("loadInfoSetMap", 
                    py::overload_cast<const string&, const string&>(
                        &regret_data_utils::SaveLoader::loadInfoSetMap<string>
                    ))
        .def_static("loadInfoSetMapInt", 
                    py::overload_cast<const string&, const string&>(
                        &regret_data_utils::SaveLoader::loadInfoSetMap<size_t>
                    ));
    
    // InfoSet Converter utilities
    py::class_<regret_data_utils::Converter>(m, "InfoSetConverter")
        .def_static("convertToString", &regret_data_utils::Converter::convertTo<string, size_t>)
        .def_static("convertToInt", &regret_data_utils::Converter::convertTo<size_t, string>);

    // Metric calculation utilities
    m.def("calculateMetricStr", &regret_data_utils::calculateMetric<string>);
    m.def("calculateMetricInt", &regret_data_utils::calculateMetric<size_t>);


    // CFRPlus classes with nested Builders
    auto cfrplus_str = py::class_<CFRPlus<string>>(m, "CFRPlusStr")
        .def(py::init<shared_ptr<const GameNode>, bool, double, const RegretDataMap<string>&>(),
             py::arg("root_node"), py::arg("initial_evaluation_run") = true,
             py::arg("e_soft_regsum_strategies") = 0.0,
             py::arg("initial_state") = RegretDataMap<string>())
        .def("evaluateAndUpdateRegretSum", &CFRPlus<string>::evaluateAndUpdateRegretSum,
             py::arg("accumulate_regsum") = true, py::arg("accumulate_strategy") = true)
        .def("evaluateRegretSum", &CFRPlus<string>::evaluateRegretSum)
    ...
    auto cfrplus_int = py::class_<CFRPlus<size_t>>(m, "CFRPlusInt")
        .def(py::init<shared_ptr<const GameNode>, bool, double, const RegretDataMap<size_t>&>(),
             py::arg("root_node"), py::arg("initial_evaluation_run") = true,
             py::arg("e_soft_regsum_strategies") = 0.0,
             py::arg("initial_state") = RegretDataMap<size_t>())
        .def("evaluateAndUpdateRegretSum", &CFRPlus<size_t>::evaluateAndUpdateRegretSum,
             py::arg("accumulate_regsum") = true, py::arg("accumulate_strategy") = true)
        .def("evaluateRegretSum", &CFRPlus<size_t>::evaluateRegretSum)
