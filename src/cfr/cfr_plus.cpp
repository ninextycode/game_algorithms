...
typename CFRPlus<RKey>::Builder& CFRPlus<RKey>::Builder::setRootNode(
...
template<typename RKey>
typename CFRPlus<RKey>::Builder& CFRPlus<RKey>::Builder::setInitialEvaluationRun(
...
template<typename RKey>
typename CFRPlus<RKey>::Builder& CFRPlus<RKey>::Builder::setESoftRegsumStrategies(
...
template<typename RKey>
typename CFRPlus<RKey>::Builder& CFRPlus<RKey>::Builder::setInitialState(
    const RegretDataMap<RKey>& initial_state
) {
...
template<typename RKey>
CFRPlus<RKey> CFRPlus<RKey>::Builder::buildCfr() {
    if (!root_node_) {
...
template<typename RKey>
CFRPlus<RKey>::CFRPlus(
    shared_ptr<const GameNode> root_node,
    bool inital_evaluation_run,
    double e_soft_regsum_strategies,
    const RegretDataMap<RKey>& initial_infosets
):
    root_node_(root_node),
...
template<typename RKey>
void CFRPlus<RKey>::initInfoStates() {
    initInfoStatesRecursively(root_node_);
}

template<typename RKey>
void CFRPlus<RKey>::initInfoStatesRecursively(const shared_ptr<const GameNode> node) {
    ...
    if (node->getType() == GameNode::Type::Decision) {
        RKey infoset_key = getInfoSetKey(node);
...
template<typename RKey>
const RegretDataMap<RKey>& CFRPlus<RKey>::getStrategyInfoSets() {
    return infosets_;
}

template<typename RKey>
double CFRPlus<RKey>::evaluateAndUpdateRegretSum(
...
template<typename RKey>
double CFRPlus<RKey>::evaluateRegretSum() {
...
template<typename RKey>
double CFRPlus<RKey>::processNode(
...
*/
template<typename RKey>
double CFRPlus<RKey>::processDecisionNode(
    const shared_ptr<const GameNode> node,
    double p_past_actions_p0,
    double p_past_actions_p1,
    double p_past_chances,
    ...
    const RKey& infoset_key = getInfoSetKey(node);

    RegretData& infoset = infosets_.at(infoset_key);
...
template<typename RKey>
double CFRPlus<RKey>::processChanceNode(
    const shared_ptr<const GameNode> node,
    double p_past_actions_p0,
    double p_past_actions_p1,
    double p_past_chances,
...
template<typename RKey>
double CFRPlus<RKey>::processTerminalNode(
    const shared_ptr<const GameNode> node
) {
