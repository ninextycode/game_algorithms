...
void regret_data_utils::SaveLoader::saveInfoSetMapToFile(
...
template <RegretDataKey Key>
void regret_data_utils::SaveLoader::saveInfoSetMapStrategy(
    const RegretDataMap<Key>& infoset_map, 
    const string& filename
) {
    saveInfoSetMapToFile(
        infoset_map, filename, 
        &RegretData::cumulative_strategy_not_norm_
    );
}

// Implement the regret sum save/load functions
template <RegretDataKey Key>
void regret_data_utils::SaveLoader::saveInfoSetMapRegretSum(
    const RegretDataMap<Key>& infoset_map, 
    const string& filename
) {
    saveInfoSetMapToFile(
        infoset_map, filename, 
        &RegretData::regret_sum_
    );
}

// Implement the instant regret save/load functions
template <RegretDataKey Key>
void regret_data_utils::SaveLoader::saveInfoSetMapInstantRegret(
    const RegretDataMap<Key>& infoset_map, 
    const string& filename
) {
    saveInfoSetMapToFile(
        infoset_map, filename, 
        &RegretData::instant_regret_
    );
}
...
template <RegretDataKey Key>
void regret_data_utils::SaveLoader::loadInfoSetMapDataFromFile(
    RegretDataMap<Key>& infoset_map,
    const string& filename,
    ...
    while (pos < content.length()) {
        ...
        if (it == infoset_map.end()) {
            ...
            RegretData infoset(values.size());
...
template <RegretDataKey Key>
RegretDataMap<Key> regret_data_utils::SaveLoader::loadInfoSetMap(
...
RegretDataMap<KeyTo> regret_data_utils::Converter::convertTo(
    const RegretDataMap<KeyFrom>& infoset_map
) {
    RegretDataMap<KeyTo> converted_map;
...
template <RegretDataKey Key>
regret_data_utils::RegretMetric regret_data_utils::calculateMetric(
    const RegretDataMap<Key>& infoset_map
) {
